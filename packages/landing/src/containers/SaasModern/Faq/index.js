...
const FaqSection = ({
  ...
  useEffect(async() => {
    const params = {
      content_type: 'faqData',
      
    }
    let client= getPage()
    let result=await client.getEntries(params)
    setFaqData(result?.items||[])
  }, [])
  ...
  return (
    <Box {...sectionWrapper} id="faq_section">
      <Container>
        <Box {...secTitleWrapper}>
          <Text {...secText} content={faqContent?.title} />
          <Heading {...secHeading} content={faqContent?.description} />
        </Box>
        <Box {...row}>
          <Box {...col}>
            <FaqWrapper>
                ...
                <>
                  {faqData.map((accordionItem, index) => (
                    <AccordionItem
                      ...
                      key={`accordion-${index}`}
                      expanded={accordionItem.fields.expend}
                    >
                      <>
                        <AccordionTitle className="accordion_title">
                          <>
                            <Heading {...title} content={accordionItem.fields.title} />
                            <IconWrapper>
                              <OpenIcon>
                        ...
                        <AccordionBody className="accordion_body">
                          <Text
                            {...description}
                            content={accordionItem.fields.description}
                          />
                        </AccordionBody>
                      </>
                    </AccordionItem>
