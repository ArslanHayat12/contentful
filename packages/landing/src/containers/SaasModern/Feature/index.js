...
const FeatureSection = ({
  ...
  useEffect(async() => {
    const params = {
      content_type: 'featureItems',
      
    }
    let   client= getPage()
    let result=await client.getEntries(params)
    console.log(result)
    setFeaturesItems(result?.items.reverse()||[])

    //   setFeaturesItems(result.items)
  ...
  return (
    <Box {...sectionWrapper} as="section" id="feature_section">
      <Container>
        <Box {...secTitleWrapper}>
          <Text {...secText} content={featureContent?.title} />
          <Heading {...secHeading} content={featureContent?.description}/>
        </Box>
          ...
          {featuresItems.map((item, index) => (
            <Box {...col} key={`feature-item-${index}`}>
              <FeatureBlock
                ...
                iconPosition="left"
                title={<Heading content={item.fields.title} {...featureTitle} />}
                description={
                  <Text content={item.fields.description} {...featureDescription} />
                }
              />
            </Box>
          ))}
